...
export async function getTopTutorsByRole(limit = 10) {
  try {
    ...
    const transformedData = data?.map(tutor => ({
      id: tutor.id,
      name: tutor.name,
      avatar: tutor.avatar_url || "/placeholder.svg?height=80&width=80",
      subject: "General Tutoring", // Default, can be enhanced to fetch from tutor_subjects
      rating: tutor.rating || 4.5,
      reviews: tutor.total_reviews || 0,
      experience: `${tutor.years_of_experience || 0}+ years`,
      location: tutor.addresses?.city ? `${tutor.addresses.city}, ${tutor.addresses.state}` : "Remote",
      hourlyRate: 50, // Default rate, can be enhanced to fetch from tutors table
      badges: ["Verified Tutor", "Top Rated"],
      totalSessions: Math.floor((tutor.total_reviews || 0) * 1.5), // Estimate sessions from reviews
      responseTime: "Within 24 hours",
      specialization: tutor.bio ? tutor.bio.substring(0, 50) + "..." : "General tutoring and academic support",
      bgColor: tutor.id % 2 === 0 ? "bg-gradient-to-br from-yellow-400 to-yellow-500" : "bg-gradient-to-br from-blue-400 to-blue-500",
      feedbackRate: `${Math.floor(85 + Math.random() * 15)}% Positive Feedback`,
      country: tutor.addresses?.country || "Global"
    })) || []
...
export async function getOpenRequests({
  ...
  try {
    ...
    return { data: transformedData, error: null, total: transformedData.length }
  } catch (error) {
