...
export async function getOrCreateUser(authUser) {
  try {
    // First, try to get existing user with address and tutor data
...
export async function getTutorSubjectsByEmail(userEmail) {
  try {
    // First get the user ID and tutor ID
    const { data: userData, error: userError } = await supabase
      .from('users')
      .select('id')
      .eq('email', userEmail)
    ...
    // Get tutor ID - use getTutorByUserId to handle creation if needed
    const tutorResult = await getTutorByUserId(userData.id)
    if (tutorResult.error || !tutorResult.data) {
      return { data: [], error: null } // Return empty array if tutor creation failed
    }
...
export async function getTutorByUserId(userId) {
  try {
    ...
    if (!data) {
      // Create tutor if it doesn't exist
