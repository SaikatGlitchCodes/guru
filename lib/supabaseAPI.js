...
export async function getTopTutorsByRole(limit = 10) {
  try {
    console.log('🔍 getTopTutorsByRole called with limit:', limit)
    ...
    // Transform the data to match the expected format - SECURE: No contact details
    const transformedData = data?.map(tutor => ({
      id: tutor.id,
      user_id: tutor.user_id,
      name: tutor.users.name,
      // SECURITY: Email removed from top tutors listing - only available after payment
...
export async function getAllTutors() {
  try {
    console.log('🔍 getAllTutors called')
    ...
    const { data: tutorData, error: tutorError } = await supabase
      ...
      // .eq('users.status', 'active')
      .order('total_sessions', { ascending: false })
    console.log('Successfully fetched', tutorData)
    if (tutorError) {
      console.error('Error fetching from tutors table:', tutorError)
      throw tutorError
    }

    // Transform the data to match expected format - SECURE: No contact details in listings
    const transformedData = tutorData?.map(tutor => ({
      id: tutor.id,
      user_id: tutor.user_id,
      name: tutor.users.name,
      // SECURITY: Email removed from public listings - only available after payment
...
export async function searchTutors({
  ...
  try {
    console.log('🔍 searchTutors called with filters:', {
    ...
    // DON'T apply pagination yet - get all data for filtering first
    const { data: allTutorData, error } = await query;
    console.log('allTutorData', allTutorData)
    if (error) {
      console.error('Database query error:', error)
      throw error;
    }

    console.log('Raw tutor data fetched:', allTutorData?.length, 'records')

    // Transform the data - SECURE: No contact details in search results
    let transformedData = allTutorData?.map(tutor => ({
      id: tutor.id,
      user_id: tutor.user_id,
      name: tutor.users.name,
      // SECURITY: Email removed from search results - only available after payment
      avatar_url: tutor.users.avatar_url || "/placeholder.svg?height=80&width=80",
    ...
    console.log(`Returning page ${page}/${totalPages}: ${paginatedData.length} tutors (${offset + 1}-${offset + paginatedData.length} of ${totalFilteredCount})`)
    ...
    return {
      ...
      currentPage: page,
      hasMore: offset + limit < totalFilteredCount,
      error: null
    };
...
export async function getTutorFiltersData() {
  try {
    console.log('🔍 getTutorFiltersData called')
...
export async function getAllTutorsLegacy() {
  console.log('🔍 getAllTutorsLegacy called')
...
export async function searchTutorsOld({
  ...
  try {
    console.log('🔍 searchTutorsOld called with filters:', {
      searchQuery, subjects, locations, minPrice, maxPrice, minRating, availabilityStatus, sortBy
    })
    let query = supabase
...
export async function getTutorProfile(tutorId) {
  try {
    console.log('🔍 getTutorProfile called with tutorId:', tutorId)
    // Get current user session
    const { data: { user: currentUser } } = await supabase.auth.getUser()

    // Step 1: Check if current user has contact access for this tutor (if user is logged in)
    let hasContactAccess = false
    let tutorEmail = null

    if (currentUser) {
      // First, get just the tutor's email to check contact access
      const { data: tutorEmailData, error: emailError } = await supabase
        .from('tutors')
        .select(`
          users!inner(email)
        `)
      ...
      if (!emailError && tutorEmailData) {
        tutorEmail = tutorEmailData.users.email
        ...
        hasContactAccess = !!contactAccess
      }
    }

    // Step 2: Query tutor data - include contact details only if user has access
    const contactFields = hasContactAccess ? ', email, phone_number' : ''

    const { data, error } = await supabase
      .from('tutors')
      .select(`
        *,
        users!inner(*${contactFields},
          addresses(*)
        ),
        tutor_subjects(
          *,
    ...
    const transformedData = {
      ...data,
      // User data - SECURE: Contact details only if access is purchased
      name: data.users.name,
      email: hasContactAccess ? data.users.email : null,
      phone_number: hasContactAccess ? data.users.phone_number : null,
      ...
      // Security flag to indicate if contact details are available
      hasContactAccess,
      ...
      // Availability data
      availability: data.tutor_availability || []
    }
...
export async function getTutorAvailability(tutorId, startDate, endDate) {
  try {
    console.log('🔍 getTutorAvailability called with:', { tutorId, startDate, endDate })
    const { data, error } = await supabase
...
export async function getOrCreateUser(authUser) {
  try {
    console.log('🔍 getOrCreateUser called with authUser email:', authUser?.email)
    // First, try to get existing user with address data only
    ...
    if (error && error.code === 'PGRST116') {
      ...
      user = newUser
      console.log('Created new user with student role:', user.email)
    } else if (error) {
      throw error
    }

    // Only fetch tutor data if user is actually a tutor
    if (user.role === 'tutor') {
      const { data: tutorData, error: tutorError } = await supabase
        .from('tutors')
        .select('*')
      ...
      // Don't throw error if tutor record doesn't exist - it will be created when needed
      if (!tutorError && tutorData) {
        user.tutor = tutorData
      } else {
        user.tutor = null
      }
    } else {
...
export async function updateUserProfile(userId, updates) {
  try {
    console.log('🔍 updateUserProfile called with userId:', userId, 'updates:', updates)
    const { data, error } = await supabase
      .from('users')
      .update({
        ...updates,
...
export async function createRequest(requestData) {
  try {
    console.log('🔍 createRequest called with data:', requestData)
    ...
    const requestInsertData = {
      ...
      price_currency: requestData.price_currency || 'INR',
...
export async function createTutorRequest(requestData) {
  try {
    console.log('🔍 createTutorRequest called with data:', requestData)
    const { data, error } = await supabase
      .from('requests')
      .insert(requestData)
      .select()
...
export async function getOpenRequests({
  ...
  try {
    console.log('🔍 getOpenRequests called with filters:', {
      searchQuery, subjects, locations, minPrice, maxPrice, meetingTypes, urgencyLevels, requestTypes, levels, sortBy, page, limit
    })
...
export async function getRequestFilterOptions() {
  try {
    console.log('🔍 getRequestFilterOptions called')
    // Get subjects
...
export async function getUserRequests(userIdentifier) {
  try {
    console.log('🔍 getUserRequests called with identifier:', userIdentifier)
...
export async function updateRequestStatus(requestId, status, userId) {
  try {
    console.log('🔍 updateRequestStatus called with:', { requestId, status, userId })
...
export async function getRequestById(requestId, tutorEmail = null) {
  try {
    console.log('🔍 getRequestById called with:', { requestId, tutorEmail })
...
export async function applyToRequest(applicationData) {
  try {
    console.log('🔍 applyToRequest called with data:', applicationData)
    const { data, error } = await supabase
      .from('applications')
...
export async function getTutorApplications(tutorId) {
  try {
    console.log('🔍 getTutorApplications called with tutorId:', tutorId)
    const { data, error } = await supabase
      .from('applications')
      .select(`
        *,
...
export async function getUserSessions(userId, role = 'student') {
  try {
    console.log('🔍 getUserSessions called with:', { userId, role })
...
export async function getConversationMessages(userId, otherUserId) {
  try {
    console.log('🔍 getConversationMessages called with:', { userId, otherUserId })
    const { data, error } = await supabase
      .from('messages')
      .select(`
        *,
...
export async function createSubject(subjectData) {
  try {
    const { data, error } = await supabase
      .from('subjects')
      .insert({ name: subjectData.name, created_at: new Date(), updated_at: new Date() })
      .select('id, name')
      .single()
...
// ===== TUTOR SUBJECTS MANAGEMENT =====

/**
 * Get tutor's subjects with their expertise details by user email
 */
export async function getTutorSubjectsByEmail(userEmail) {
  try {
    console.log('🔍 getTutorSubjectsByEmail called with email:', userEmail)
    
    // First get the user ID and check if they are a tutor
    const { data: userData, error: userError } = await supabase
      .from('users')
      .select('id, role')
      .eq('email', userEmail)
    ...
    // Only proceed if user is actually a tutor
    if (userData.role !== 'tutor') {
      console.log('User is not a tutor, returning empty subjects array')
      return { data: [], error: null }
    }

    // Get tutor ID - only for actual tutors
    const tutorResult = await getTutorByUserId(userData.id)
    if (tutorResult.error || !tutorResult.data) {
      console.log('No tutor record found for tutor user, returning empty array')
      return { data: [], error: null }
    }
...
/**
 * Add subject to tutor with proficiency details by user email
 */
export async function addTutorSubjectByEmail(tutorId, subjectId, proficiencyData = {}) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        tutor_id: tutorId,
        subject_id: subjectId,
        proficiency_level: proficiencyData.proficiency_level || 'intermediate',
        years_experience: proficiencyData.years_experience || 0,
        hourly_rate_override: proficiencyData.hourly_rate_override || null
      }])
      .select(`
        *,
        subjects (
...
/**
 * Update tutor subject proficiency details
 */
export async function updateTutorSubjectByEmail(tutorSubjectId, proficiencyData) {
  try {
    const { data, error } = await supabase
      .from('tutor_subjects')
      .update(proficiencyData)
...
/**
 * Remove subject from tutor by user email
 */
export async function removeTutorSubjectByEmail(tutorId, subjectId) {
  try {
    const { error } = await supabase
      .from('tutor_subjects')
      .delete()
...
/**
 * Get tutor information by user ID
 */
export async function getTutorByUserId(userId) {
  try {
    console.log('🔍 getTutorByUserId called with userId:', userId)
    
    const { data, error } = await supabase
      .from('tutors')
      .select('id, user_id, hourly_rate, experience_years, verified')
      .eq('user_id', userId)
      .single()
    ...
    if (!data) {
      // First check if the user is actually a tutor before creating tutor record
      const { data: userData, error: userError } = await supabase
        .from('users')
        .select('role')
        .eq('id', userId)
      ...
      // Only create tutor record if user role is 'tutor'
      if (userData.role !== 'tutor') {
        console.log('User is not a tutor, not creating tutor record')
        return { data: null, error: { message: 'User is not a tutor' } }
      }

      // Create tutor record only for users with tutor role
      ...
      console.log('Created tutor record for tutor user:', newTutor)
      return { data: newTutor, error: null }
    }
...
export async function createProfile(profileData) {
  try {
    console.log('🔍 createProfile called with data:', profileData)
...
export async function updateProfile(profileData, email) {
  try {
    console.log('🔍 updateProfile called with email:', email, 'data:', profileData)
    // First get the current user to check for existing address and get user ID
    const { data: currentUser } = await supabase
      .from('users')
      .select('id, address_id, role')
      .eq('email', email)
      .single()
    ...
    // If user is a tutor and tutor data is provided, handle tutor profile
    if (profileData.role === 'tutor' && profileData.tutor) {
      // Get or create tutor profile
      const tutorResult = await getTutorByUserId(currentUser.id)
      if (tutorResult.error) {
        throw tutorResult.error
      }

      const tutorData = {
        hourly_rate: profileData.tutor.hourly_rate,
        experience_years: profileData.tutor.experience_years,
        education: profileData.tutor.education,
        certifications: profileData.tutor.certifications,
        languages: profileData.tutor.languages,
        teaching_style: profileData.tutor.teaching_style,
        specializations: profileData.tutor.specializations,
        response_time: profileData.tutor.response_time,
        availability_status: profileData.tutor.availability_status,
        verified: profileData.tutor.verified,
        verification_documents: profileData.tutor.verification_documents,
        background_check: profileData.tutor.background_check,
        preferred_meeting_types: profileData.tutor.preferred_meeting_types,
        travel_radius_km: profileData.tutor.travel_radius_km,
        minimum_session_duration: profileData.tutor.minimum_session_duration,
        cancellation_policy: profileData.tutor.cancellation_policy,
        instant_booking: profileData.tutor.instant_booking,
        updated_at: new Date().toISOString()
      }

      // Remove undefined values
      Object.keys(tutorData).forEach(key =>
        tutorData[key] === undefined && delete tutorData[key]
      )
      ...
      // Fetch the complete updated profile with tutor data
  ...
  try {
    ...
    if (uploadError) throw uploadError
      ...
      const { error: updateError } = await supabase
        .from('users')
        .update({
          avatar_url: 'https://uwqkktncakctujlwbacq.supabase.co/storage/v1/object/public/profileImg/' + fileName,
          updated_at: new Date().toISOString()
        })
        .eq('email', user);
...
export async function contactStudent(tutorEmail, requestId, coinCost) {
  try {
    console.log('🔍 contactStudent called with:', { tutorEmail, requestId, coinCost })
    ...
    // Get student information for the redirect
    ...
    // Return contact info including student ID for redirect
    const contactInfo = {
      id: requestData?.user?.id || null,
      email: requestData?.user?.email || null,
      phone: requestData?.user?.phone_number || null,
      name: requestData?.user?.name || null
    };

    console.log('🔍 contactStudent returning contactInfo:', contactInfo)
    return { data, error: null, contactInfo };
  } catch (error) {
...
export async function getTutorStudents(tutorEmail) {
  try {
    const { data, error } = await supabase
      ...
      .order('contacted_at', { ascending: false });
    if (error) throw error;
        console.log('Fetched tutor students successfully:', data);
 ...
 */
export async function createPaymentTransaction(transactionData, profile) {
  try {
    ...
    if (transactionData.status === 'succeeded') {
      ...
      const { data: updatedUser, error: updateError } = await supabase
        .from('users')
        .update({ coin_balance: profile.coin_balance + transactionData.coins })
        .eq('id', profile.id)
        .select()
        .single()

      console.log('Added coins to user:', updatedUser)
...
export async function getTutorProfileEnhanced(tutorId) {
  try {
    ...
    // Step 1: Check if current user has contact access (if user is logged in)
    let hasContactAccess = false
    let tutorEmail = null
    ...
    // Step 2: Query full tutor data - include contact details only if user has access
    const contactFields = hasContactAccess ? ', email' : ''
    ...
    // Transform the data for better frontend consumption - SECURE
    const transformedData = {
      id: data.id,
...
/**
 * Get user coins and contact access information
 */
export async function getUserCoins(userId) {
  try {
    ...
    // Get tutors this user has contact access to
    const { data: contactAccess, error: contactError } = await supabase
      .from('contact_activities')
      .select('tutor_email, request_id')
      .eq('user_id', userId)

    if (contactError && contactError.code !== 'PGRST116') { // PGRST116 = no rows found
      throw contactError
    }

    // Create a map of tutor access by email
    const hasContactAccess = {}
    if (contactAccess) {
      contactAccess.forEach(contact => {
        hasContactAccess[contact.tutor_email] = true
      })
    }
    ...
    return { coins: 0, hasContactAccess: {}, error }
  }
}

/**
 * Purchase contact access for a tutor
 */
export async function purchaseContact(tutorId, coinCost) {
  try {
    ...
    const { data: existingContact } = await supabase
      ...
      .select('id')
      .eq('user_id', user.id)
      .eq('tutor_email', tutor.users.email)
      .single()

    if (existingContact) {
      return { data: { alreadyPurchased: true }, error: null }
    }
    ...
    const deductResult = await deductCoins(
      user.id,
      coinCost,
      `Contact access for tutor: ${tutor.users.name}`
    )
    ...
    const { data: contactActivity, error: contactError } = await supabase
      ...
      .insert({
        user_id: user.id,
        tutor_email: tutor.users.email,
        request_id: null, // This can be updated if needed
        coin_cost: coinCost
      })
      .select()
      .single()
